...
var AppComponent = /** @class */ (function () {
    ...
    AppComponent.prototype.expandSelectedScenario = function (selected, sandboxIndex) {
        // Find the index of Category/Label of the selected sandbox
        var categoryIndex = this.uniqueLabels.indexOf(this.filteredSandboxMenuItems[sandboxIndex].label);
        var groupIndex = 0;
        // Search through stored Scenario names to find the Group index
        for (var i = 0; i < this.uniqueGroups[sandboxIndex].groupItems.length; i++) {
            if (this.uniqueGroups[sandboxIndex].groupItems[i].indexOf(this.filteredSandboxMenuItems[sandboxIndex].scenarioMenuItems[selected.scenarioKey - 1].description) !== -1) {
                groupIndex = i;
                break;
            }
        }
